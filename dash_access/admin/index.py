...
admin_layout = dbc.Container(dbc.Row(dbc.Col(
    [
                    ...
                    [
                        ...
                        dbc.Col(
                            [
                                ...
                                dbc.Input(id=id("granted-input"),list=id("granted-datalist"),placeholder="Name..."),
                                dbc.Button('Add to Transaction',id=id("granted-add-button"),color='secondary',block=True)
                            ],
                            width=5,
                        ),
                    ],
                    ...
                    [
                        ...
                        dbc.Col(
                            [
                                ...
                                dbc.Input(id=id("principal-input"),list=id("principal-datalist"),placeholder="Name..."),
                                dbc.Button('Add to Transaction',id=id("principal-add-button"),color='secondary',block=True)
                            ],
                            width=5,
                        ),
                    ],
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    [
                        dbc.Button("Submit Transaction",id=id("submit-transaction"),color='success',block=True),
...
def register_admin_callbacks(app: dash.Dash):
    ...
    def POPULATE_PRINCIPAL_DATALIST(n,principal_type):
        return [
            html.Option(value=x)
            for x in (
                set([row['principal'] for row in session['dash-access-admin-values'] if row['principal_type']==principal_type]) 
    ...
    def POPULATE_GRANT_DATALIST(n,granted_type):
        return [
            html.Option(value=x)
            for x in (
                set([row['granted'] for row in session['dash-access-admin-values'] if row['granted_type']==granted_type]) 
                if 'dash-access-admin-values' in session 
                else ['test']
            )
        ]
    ...
    def ADD_GRANTS(n_clicks, clear, input, type, added_data):
        ...
        list_data = sorted(unique_added, key=lambda x: (x['granted_type'],x['granted']))
        list_view = [dbc.ListGroupItem(f"{x['granted_type']}: {x['granted']}") for x in added_data]
    ...
    def ADD_PRINCIPALS(n_clicks, clear, input, type, added_data):
        ...
        unique_added =  [json.loads(x) for x in set([json.dumps(x) for x in added_data])]
        list_data = sorted(unique_added, key=lambda x: (x['principal_type'],x['principal']))
        list_view = [dbc.ListGroupItem(f"{x['principal_type']}: {x['principal']}") for x in added_data]
        return list_view, json.dumps(list_data), None
    ...
    def SUBMIT_TRANSACTION(n_clicks,principal, granted):
        ...
        return no_update, dbc.Alert("Something went wrong, try again?", color='danger', dismissable=True)
